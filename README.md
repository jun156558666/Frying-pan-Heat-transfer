...
## 質量特性（SolidWorks アセンブリ・デフォルト座標系）
...
> 質量 / 体積 ≈ 1.00 g/cm³（SolidWorks デフォルト材料のまま）。シミュレーションでは材料ごとの正しい密度を使用しているため影響なし。

### 慣性モーメント（重心基準・出力座標系と整列）単位: g·mm²
...
### 慣性モーメント（出力座標系原点基準）単位: g·mm²

|  | x | y | z |
|---|---|---|---|
| **x** | 913,761 | 778,846 | 0 |
| **y** | 778,846 | 6,700,765 | 0 |
| **z** | 0 | 0 | 6,168,098 |

### 慣性主モーメント（重心基準）単位: g·mm²

| 軸 | 方向ベクトル | 主モーメント |
|---|---|---|
| Ix | (1.00, 0.08, 0.00) | 772,638 |
| Iy | (0.00, 0.00, −1.00) | 4,249,117 |
| Iz | (−0.08, 1.00, 0.00) | 4,922,906 |
